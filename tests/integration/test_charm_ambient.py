...
from charms_dependencies import ISTIO_BEACON_K8S, ISTIO_INGRESS_K8S, ISTIO_K8S, MLMD
...
async def test_virtual_service(ops_test, lightkube_client):
    ...
    # TODO: verify network is being redirected as expected


# def assert_httproute_exists(name: str, namespace: str, lightkube_client: Client):
#     """Will raise a ApiError(404) if the virtualservice does not exist."""
#     log.info(f"Asserting that  VirtualService '{name}' exists.")
#     http_route_lightkube_resource = create_namespaced_resource(
#         group="gateway.networking.k8s.io",
#         version="v1",
#         kind="HTTPRoute",
#         plural="httproutes",
#     )
#     routes = lightkube_client.list(http_route_lightkube_resource, namespace=namespace)
#     for route in routes:
#         route_name = route["metadata"]["name"]
#         if name in route_name:
#             return
#         log.info(f"HTTPRoute '{route_name}' exists.")
